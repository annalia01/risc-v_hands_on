...
extern uint8_t i[] __attribute__((
...
// Verify the matrices
int verify_matrix(int8_t *matrix, int8_t *golden_matrix, int64_t R,
                  int64_t C) {
  for (int r = 0; r < R; ++r)
    for (int c = 0; c < C; ++c)
...
int main() {
  ...
  if (F == 3)
    ...
    iconv2d_5x5_uint8(o, i, f, M, N, F);
  else if (F == 7)
    iconv2d_7x7_uint8(o, i, f, M, N, F);
  else
