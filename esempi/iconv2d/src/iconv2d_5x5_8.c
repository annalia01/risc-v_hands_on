...
void iconv2d_5x5_uint8(int8_t *o, uint8_t *i, int8_t *f, int64_t R, int64_t C,
  ...
  for (int32_t r = 0; r < R; r += block_size_o) {
    ...
    iconv2d_vec_4xC_5x5_uint8(o_, i, f, C, F, r);
...
int32_t debug_buf_v0[64] __attribute__((aligned(32)));
  ...
  asm volatile("vle8.v v24, (%0); add %0, %0, %1" : "+&r"(i_) : "r"(ldi));
  asm volatile("vle8.v v28, (%0); add %0, %0, %1" : "+&r"(i_) : "r"(ldi));
